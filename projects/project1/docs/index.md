...
The synthesizer's output is mapped to the kinematics, then converted to a circular motion. We implented a change line function where the plotter will go to the next grid and start a new circle.
...
This is a great opportunity to understand how signal flows in the stepdance system, and it's very interesting to have a unusual input to the stepdance as a starting point to utilize the conecpt of mapping in an artistic way. Also very happy to use the plotter as a medium not just focused on drawing specific shapes but on the process of drawing.

If given more time, what we would improve is, first, as we realized in the presentation, the range of the signal has exceeded the resolution of the system, and that's why we're not getting as much continuous lines as before we made the movement circular. We could calibrate the voltage and make the result less like an on/off switch. Secondly we realized the interface right now, which is directly manipulating a synthesizer is not the most user-friendly one and it's hard to have some that does not know how to use this specific synthesizer to be confident enough to try it out.
